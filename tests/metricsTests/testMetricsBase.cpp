...
public:
    ...
    static bool checkCorrectness(MetricsBase::testimg_entry &entry) { return entry.is_correct; }
};
...
TEST(_choose_classes, _chooses_topN_unique_classes)
{
    ...
    std::vector<MetricsBase::testimg_entry> img_entry_vec = {img_entry1, img_entry2};
    ...
    float val_correct = std::count_if(img_entry_vec.begin(), img_entry_vec.end(), MetricsBaseWrapper::checkCorrectness);
