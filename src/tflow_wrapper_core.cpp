...
tensorflow::SessionOptions TensorflowWrapperCore::configureSession()
{
    ...
    if (_cpu_only) {
        ...
        if (device_map) {
            tf_base::DebugOutput("Warning", "Disabling GPU!!!");
            (*device_map)["GPU"] = 0;
        }
    } else {
...
bool TensorflowWrapperCore::load(const std::string &filename, const std::string &inputNodeName) {
    ...
    if (!status.ok()) {
        tf_base::DebugOutput("tf error: ", status.ToString());
    ...
    if (!status.ok()) {
        ...
        tf_base::DebugOutput("Graph successfully loaded!", "");
    }
    parseName(filename);
...
tensorflow::Tensor TensorflowWrapperCore::getTensorFromGraph(const std::string &tensor_name)
{
    ...
    tf_base::DebugOutput("Sucessfully run graph! Status is: ", status.ToString());
